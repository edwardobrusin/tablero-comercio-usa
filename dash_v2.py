...
# ==========================================
# 5. EXTRACCIÓN ON-DEMAND CON DUCKDB (LÓGICA YTD)
# ==========================================
capitulos_validos = HTS_SECTIONS[seccion_sel]

# Extraemos la metadata temporal sin cargar el dataset completo
...
# Formateamos las variables para inyectarlas de forma segura en la consulta SQL
caps_sql = ", ".join([f"'{c}'" for c in capitulos_validos])
meses_sql = ", ".join([str(m) for m in meses_list])

# Ejecutamos el filtro estructural directamente sobre el Parquet en disco.
# El casteo estricto (CAST a VARCHAR) garantiza la conservación de ceros a la izquierda.
query_base = f"""
    SELECT 
    ...
    WHERE STATE = '{selected_state_code}'
      AND Chapter IN ({caps_sql})
      AND year IN ({max_year}, {max_year - 1})
      AND month IN ({meses_sql})
"""

# Únicamente la porción de datos solicitada por el usuario ingresa a la memoria RAM
...
def get_full_year_total_bar(flow_type, color_mex):
    # Ya que df_tot y df_mex ya no están en memoria de forma global, calculamos los 
    # totales del año previo (los 12 meses, no solo el YTD) llamando directamente a DuckDB
    caps_sql = ", ".join([f"'{c}'" for c in capitulos_validos])
    
    query_tot = f"SELECT SUM(VALOR) FROM '{RUTA_TOT}' WHERE STATE = '{selected_state_code}' AND Chapter IN ({caps_sql}) AND flow = '{flow_type}' AND year = {max_year - 1}"
    query_mex = f"SELECT SUM(VALOR) FROM '{RUTA_MEX}' WHERE STATE = '{selected_state_code}' AND Chapter IN ({caps_sql}) AND flow = '{flow_type}' AND year = {max_year - 1}"
...
def get_top_10_total_split(flow_type, color_mex, color_rest):
    ...
    # Groupby integrando la descripción que ahora viene de la base
